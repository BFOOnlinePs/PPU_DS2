...
use App\Imports\integration_company\UserImport;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class IntegrationCompaniesController extends Controller
{
    public function index()
    {
        return view('project.admin.settings.integration_company.index');
    }
    ...
    public function submitForm(Request $request)
    {
        if ($request->hasFile('input-file')) {
            ...
            $user_object = new UserImport($result);
            Excel::import($user_object, $path);
            return response()->json([
                'cnt' => $user_object->getCount() ,
                'user_object_array' => $user_object->getCompanyArray()
            ]);
        }
    }
}
